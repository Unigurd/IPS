...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  ...
  | Negate (e1, _) ->
      let t1 = newName "negate_R"
      let code1 = compileExp e1 vtable t1
      code1 @ code1 @ [Mips.SUB (place, "R0",t1)]
  ...
  *)
  | Replicate (n_exp, a_exp, elem_type, (line,_)) ->
      let size_reg = newName "size_reg"
      let val_reg  = newName "val_reg"
      let n_code = compileExp n_exp vtable size_reg
      let a_code = compileExp a_exp vtable val_reg
      (* size_reg is now the integer n and val_reg is the value to be copied. *)
